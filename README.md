...
Создайте папку для проекта, я рекомендую /var/www/default, зайдите в эту папку и выполните ```git clone https://github.com/login9000/repository_11 .``` таким образом в этой папке появятся все необходимые файлы проекта. Обратите внимание на папку /var/www/default/sources-frontend в ней находятся исходники фронтенда, фронтенд написан на Angular, можно скопировать эту папку куда нибудь в другое место и уже там из другого места работать над исходниками фронтенда, если не удобно с ней работать там где она находится в данный момент. Допустим вы ее решили скопировать в /root/ таким образом если зайти в /root/sources-frontend мы увидим исходники фронтенда, и оттуда будем работать, отлично 🤔 🤔 🤔
...
Если вы установили проект на своем локальном компуктере или на VDS сервере и собираетесь просто протестить то откройте /root/sources-frontend/src/app/core/constants/api-url.ts и замените "8070" - это порт на котором на вашем локальном компуктере или на тестовом VDS сервере будет запущен nginx, если он у вас будет запущен на другом порту то укажите другой порт, вместо "8070".
...
Если собираетесь тестировать все это а не сразу закидывать на продакшен то зайдите в папку куда мы скопировали исходники фронтенда , которые на Angular , предпологается что это папка /root/sources-frontend выполните 
```
npm install
```
...
Затем откройте http://x.x.x.x:4200/ - перед вами фронтенд проекта запущенного из исходников. Чтобы увидеть проект запущенный из не исходников (из скомпилированного билда) откройте http://x.x.x.x:8070/

где: x.x.x.x - ip вашего сервера, если вы устанавливаете проект на своем локальном компуктере то вместо x.x.x.x - это скорее всего нужно вписать "127.0.0.1" 🤔 🤔 🤔
"8070" - порт на котором будет запущен nginx, если он у вас будет запущен на другом порту то укажите другой порт, вместо "8070".

### Сборка проекта 

Затем откройте /var/www/default/app/Http/Middleware/VerifyCsrfToken.php и закомментируйте все что разкомментировано внутри массива в переменной $except
Так же рекомендую удалить папку /root/sources-frontend/dist/frontend потому что в ней появится скомпилированные билды и старое все можно удалить нахуй чтобы не накапливался мусор.

Выполните:
```
ng build --output-hashing=all --configuration=production
```
в итоге собранный проект (скомпилированный билд) будет находится в /root/sources-frontend/dist/frontend нужно будет просто скопировать все это содержимое и поместить в /var/www/default/public Ну и желательно наверное сами исходники из /root/sources-frontend/src скопировать потом не забыть в /var/www/default/sources-frontend чтобы потом когда вы все выложите на гитхаб то туда помимо сборки попали еще и исходники, на Angular которые. А то может так получится что на гитхаб все что надо выложили а про исходники забыли 😂 😂 
...
chown vasya:www-data -R /var/www/default
```
(замените "vasya" на имя того пользователя которого есть в системе и  от имени которого работает у вас бекенд этого проекта)

Откройте http://x.x.x.x:8070/update_product_catalog_json чтобы обновился product_catalog.json который находится в корне, так как когда вы будете копировать все содержимое из /root/sources-frontend/dist/frontend в /var/www/default/public то  он автоматически будет заменен и поэтому его нужно обновить, просто перейтя на http://x.x.x.x:8070/update_product_catalog_json 
где: x.x.x.x - ip вашего сервера, если вы устанавливаете проект на своем локальном компуктере то вместо x.x.x.x - это скорее всего нужно вписать "127.0.0.1" 🤔 🤔 🤔
"8070" - порт на котором будет запущен nginx, если он у вас будет запущен на другом порту то укажите другой порт, вместо "8070".

И затем откройте уже http://x.x.x.x:8070/ чтобы увидеть что получилось в собранном виде.
где: x.x.x.x - ip вашего сервера, если вы устанавливаете проект на своем локальном компуктере то вместо x.x.x.x - это скорее всего нужно вписать "127.0.0.1" 🤔 🤔 🤔
"8070" - порт на котором будет запущен nginx, если он у вас будет запущен на другом порту то укажите другой порт, вместо "8070".
