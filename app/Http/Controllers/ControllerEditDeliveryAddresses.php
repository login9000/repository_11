...
class ControllerEditDeliveryAddresses extends Common{
	...
	public function __invoke(Request $request){
		...
		list($result, $err) = parent::post_request_to_api_1c('edit_delivery_addresses', ['id' => $id, 'address_str' => $address_str, 'region' => $region, 'area' => $area, 'city' => $city, 'index' => $index, 'street' => $street, 'house' => $house, 'frame' => $frame, 'letters' => $letters, 'stock' => $stock, 'apartment' => $apartment, 'fio' => $fio, 'phone' => $phone, 'token' => $token]);
		if($err){
		...
		list($data_crypt, $symmetric_key_crypt, $err) = parent::handler_data_crypt2(['delivery_addresses_id' => $id, 'full_delivery_addresses' => $address_str, 'region' => $region, 'area' => $area, 'city' => $city, 'index' => $index, 'street' => $street, 'house' => $house, 'frame' => $frame, 'letters' => $letters, 'stock' => $stock, 'apartment' => $apartment, 'is_default' => $is_default, 'fio' => $fio, 'phone' => $phone], $client_rsa_pubkey);
		if($err){
			parent::prepare_response(['error'=>$err]);
		}
		
		parent::prepare_response(['response' => ['data_crypt' => $data_crypt, 'symmetric_key_crypt' => $symmetric_key_crypt]], true);
		//parent::prepare_response(['response' => ['delivery_addresses_id' => $id, 'full_delivery_addresses' => $address_str, 'region' => $region, 'area' => $area, 'city' => $city, 'index' => $index, 'street' => $street, 'house' => $house, 'frame' => $frame, 'letters' => $letters, 'stock' => $stock, 'apartment' => $apartment, 'is_default' => $is_default, 'fio' => $fio, 'phone' => $phone]], true);
