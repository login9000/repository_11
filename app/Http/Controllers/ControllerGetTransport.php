...
class ControllerGetTransport extends Common{
	...
	public function __invoke(){
		...
		$result = parent::get_transport($token);
		
		if(array_key_exists('error', $result)){
			parent::prepare_response(['error'=>$result['error']]);
		}
		
		list($data_crypt, $symmetric_key_crypt, $err) = parent::handler_data_crypt2($result, $client_rsa_pubkey);
		if($err){
			parent::prepare_response(['error'=>$err]);
		}
		
		parent::prepare_response(['response' => ['data_crypt' => $data_crypt, 'symmetric_key_crypt' => $symmetric_key_crypt]], true);
		// parent::prepare_response(['response' => $result], true);
