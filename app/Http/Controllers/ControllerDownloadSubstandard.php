...
class ControllerDownloadSubstandard extends Common{
	...
	public function __invoke(){
		...
		list($result, $err) = parent::prepare_result_substandard_catalog($main_counterparty_id, $shipping_warehouse_id, $products, $profile, $thickness, $coating, $color);		
		if($err){
		...
		foreach($result['data'] as $c){
			...
			$substandard_data[] = [$c['НоменклатураНаименование'], $c['Характеристика'], $c['ЕдиницаИзмеренияНаименования'], (string) $c['Количество'], ($total == -1 ? '-' : $total), $prices, (string) $c['Сумма']];
			
		}
		
		array_unshift($substandard_data , ['Номенклатура', 'Длина, мм', 'Ед.Изм.', 'Кол-во', 'Итого', 'Цена', 'Сумма, ₽']);
		
		if(!is_dir($this->document_root.'/user_files/'.$user_myid)){
			mkdir($this->document_root.'/user_files/'.$user_myid, 0774);
		}

		if(!is_dir($this->document_root.'/user_files/'.$user_myid.'/substandard')){
			mkdir($this->document_root.'/user_files/'.$user_myid.'/substandard', 0774);
		} 
		...
		$data = ['shipping_warehouse_name' => str_replace(['&quot;', '&lt;', '&gt;'], ['"', '<', '>'], $shipping_warehouse_name), 'products' => $products, 'profile' => $profile, 'thickness' => $thickness, 'coating' => $coating, 'color' => $color, 'actual_date' => parent::convert_format_date3($actual_date), 'substandard' => $substandard_data];
