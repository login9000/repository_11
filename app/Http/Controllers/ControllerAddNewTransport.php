...
class ControllerAddNewTransport extends Common{
	...
	public function __invoke(Request $request){
		...
		list($result, $err) = parent::post_request_to_api_1c('add_new_transport', ['brand' => $brand, 'license_plate' => $license_plate, 'vehicle_type' => $vehicle_type, 'token' => $token]);
		if($err){
		...
		if(!array_key_exists('ТранспортноеСредствоИД', $result)){
			parent::prepare_response(['error'=>'NO_EXISTS_KEY_ТранспортноеСредствоИД']);
		}
		
		list($data_crypt, $symmetric_key_crypt, $err) = parent::handler_data_crypt2(['id' => $result['ТранспортноеСредствоИД'], 'brand' => $brand, 'license_plate' => $license_plate, 'vehicle_type' => $vehicle_type], $client_rsa_pubkey);
		if($err){
			parent::prepare_response(['error'=>$err]);
		}
		
		parent::prepare_response(['response' => ['data_crypt' => $data_crypt, 'symmetric_key_crypt' => $symmetric_key_crypt]], true);
		//parent::prepare_response(['response' => ['id' => $result['ТранспортноеСредствоИД'], 'brand' => $brand, 'license_plate' => $license_plate, 'vehicle_type' => $vehicle_type]], true);
