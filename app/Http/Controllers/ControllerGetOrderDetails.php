...
class ControllerGetOrderDetails extends Common {
	...
	public function __invoke() {
		...
		foreach ($order_details as $key => $val) {
			...
			if ($key != 'Запасы') {
				...
				if ($key == 'ДатаВыгрузки') {
					$order_details2['ДатаВыгрузки'] = str_replace('01 янв 0001', '', parent::convert_format_date2($val));
				}
				...
				if ($key == 'Дата' || $key == 'ДатаОтгрузки') {
					$order_details2[$key] = str_replace('01 янв 0001', '', parent::convert_format_date2($val));
				}
			...
			if ($key == 'Запасы') {
				...
				$order_details2['ОтветственныйОтКлиента'] = $fio1;
				$order_details2['ОтветственныйSokrof'] = $manager_fio1;
		...
		list($data_crypt, $symmetric_key_crypt, $err) = parent::handler_data_crypt2(['order_details' => ['data' => $order_details2]], $client_rsa_pubkey);
		if($err){
			parent::prepare_response(['error'=>$err]);
		}
		
		parent::prepare_response(['response' => ['data_crypt' => $data_crypt, 'symmetric_key_crypt' => $symmetric_key_crypt]], true);
		// parent::prepare_response(['response' => ['order_details' => ['data' => $order_details2]]], true);
