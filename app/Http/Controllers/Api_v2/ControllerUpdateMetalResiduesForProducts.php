...
class ControllerUpdateMetalResiduesForProducts extends Common{
	
	public function handler(Request $request){
		
		parent::check_allowed_method_api('PUT');
		...
		try{
			DB::update('UPDATE `metal_residues_for_products` SET `data` = :data WHERE `id` = 1 LIMIT 1', ['data' => $data]);
