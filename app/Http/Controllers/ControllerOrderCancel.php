...
class ControllerOrderCancel extends Common
{
	...
	public function __invoke(Request $request)
	{
		...
		try {
			...
			foreach ($result as $row) {
				if($row->status != 'in_processing'){
					parent::prepare_response(['error' => 'Заказ с таким статусом нельзя отменить']);
				}
			}
		...
		list($result, $err) = parent::post_request_to_api_1c('order_cancel', ['order_id' => $order_id, 'token' => $token]);
		if ($err) {
		...
		try {
			...
			DB::beginTransaction();
				...
				DB::update('UPDATE `orders_' . $main_counterparty_id . '` SET `status` = \'canceled\' WHERE `order_id` = :order_id LIMIT 1', ['order_id' => $order_id]);
